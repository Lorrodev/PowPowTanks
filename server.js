...
io.on('connection', function(socket) {
	...
	socket.on('changeRoom', function(lastId, newId, nick){
		if(checkExistence(newId) && rooms[newId].numPlayers < 4){
			if(checkExistence(lastId)){
			...
			try{
				if(checkExistence(newId) && checkExistence(newId, 'p', rooms[newId].players[rooms[newId].numPlayers-1])){
	...
	socket.on('leaveRoom', function(roomId, player){
		if(roomId != "Welcome" && checkExistence(roomId)){
			...
			try{
				if(checkExistence(roomId, 'p', player-1)){
	...
	socket.on('changeMap', function(roomId, lvl, player){
		...
		if(checkExistence(roomId) && !rooms[roomId].activeGame){
			init(roomId, lvl);
		}
	});
	...
	socket.on('sendMessage', function(roomId, message, player){
		try{
			...
			if(checkExistence(roomId) && checkExistence(roomId, 't', player-1)){
...
function checkExistence(id, toCheck = 'r', playerOrTank = null){ 
	switch(toCheck){
		...
		//check if rooms[id].players or specific player exists
		case 'p':
		if(rooms[id] != null){
			if(rooms[id].players.length != 0 && playerOrTank != null){
				if(rooms[id].players[playerOrTank] != null){
					return true;
				}else{
					return false;
					console.log("Player "+playerOrTank+" in room "+id+" no longer accessible");
				}
			}else if(rooms[id].players.length != 0 && playerOrTank == null){
				return true;
			}else{
				return false;
					console.log("Player "+playerOrTank+" in room "+id+" no longer accessible");
			...
			return false;
			console.log("Checked for existence of Player but room "+id+" does not exist");
		}
		break;


		//check if rooms[id].tanks or specific tank exists
		case 't':
		if(rooms[id] != null){
			if(rooms[id].tanks.length != 0 && playerOrTank != null){
				if(rooms[id].tanks[playerOrTank] != null){
					return true;
				}else{
					return false;
					console.log("Tank "+playerOrTank+" in room "+id+" no longer accessible");
				}
			}else if(rooms[id].tanks.length != 0 && playerOrTank == null){
				return true;
			}else{
				return false;
					console.log("Tank "+playerOrTank+" in room "+id+" no longer accessible");
			...
			return false;
			console.log("Checked for existence of Tank but room "+id+" does not exist");
		}
		break;
	}
}
	...
	if(checkExistence(id)){
		if (checkWin(id)){
			if(rooms[id].winner != "Draw"){
				...
				var text = "<span><span style='color: #ffbb00'><b>Draw!</b></span></span>";
				io.to(id).emit('newMessage', text);
			}
...
function checkWin(id){
	...
	if(killCount == (rooms[id].numPlayers-1)){
		...
		return true;
	}else if(killCount == (rooms[id].numPlayers)){
		rooms[id].winner = "Draw";
		return true;
	}else{
		return false;
	}
